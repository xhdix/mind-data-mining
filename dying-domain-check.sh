#!/bin/bash
#
# origin: https://github.com/bassosimone/aladdin/blob/master/domain-check.bash
#
...
doh_cache="-ODNSCache=cloudflare-dns.com 104.16.248.249 104.16.24.249"
...
function urlgetter() {
  run ./miniooni -v $extra_options -o $report_file -Asession=$uuid "$@" urlgetter
}
...
function main() {
  ...
  checking "for TLS-triggered blocking"
  urlgetter -Astep=tls_blocking \
            "-ODNSCache=$test_helper $(cat $ipv4_doh_list)" \
            -ihttps://$test_helper/
  { [ "$(getfailure)" = "ssl_invalid_hostname" ] && log "no"; } || maybe
